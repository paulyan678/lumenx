...
class TestProviderRegistryRouting:
    ...
    def test_gpt_image_and_seedance_can_route_to_newapi(self):
        registry = get_default_provider_registry()
        env = {
            "GPT_IMAGE_PROVIDER_MODE": "newapi",
            "SEEDANCE_PROVIDER_MODE": "newapi",
        }

        assert registry.resolve_backend("gpt-image-2", env=env) == "newapi"
        assert registry.resolve_backend("seedance-2.0-i2v", env=env) == "newapi"
        assert registry.resolve_backend("gpt-image-2", env={}) == "mulerouter"
        assert registry.resolve_backend("seedance-2.0-i2v", env={}) == "mulerouter"
...
class TestProviderRoutingConfig:
    def test_provider_modes_default_to_dashscope(self):
        ...
        assert config.GPT_IMAGE_PROVIDER_MODE == ProviderBackend.MULEROUTER
        assert config.SEEDANCE_PROVIDER_MODE == ProviderBackend.MULEROUTER

    def test_media_provider_modes_accept_newapi_override(self):
        config = ProviderRoutingConfig(
            GPT_IMAGE_PROVIDER_MODE="newapi",
            SEEDANCE_PROVIDER_MODE="newapi",
        )

        assert config.GPT_IMAGE_PROVIDER_MODE == ProviderBackend.NEWAPI
        assert config.SEEDANCE_PROVIDER_MODE == ProviderBackend.NEWAPI
