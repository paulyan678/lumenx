    ...
    const modal = (
        <>
            ...
            <div
                ...
                className="fixed left-1/2 top-1/2 z-[61] flex h-[88vh] w-[min(1200px,94vw)] -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-[12px] border border-foreground/10 bg-surface shadow-[0_24px_60px_-22px_rgba(0,0,0,0.9)] motion-safe:animate-[compareModalIn_240ms_cubic-bezier(0.22,1,0.36,1)_both]"
            >
                {/* Header */}
                <header className="flex shrink-0 items-center justify-between gap-3 border-b border-foreground/10 px-4 py-3">
                ...
                <div className={`grid flex-1 gap-2 overflow-hidden p-3 ${gridClass}`}>
                    {slots.map((task, i) => {
                        ...
                        return (
                            <div
                                key={task.id}
                                className="relative overflow-hidden rounded-md border border-foreground/8 bg-black"
                            >
                                {url ? (
                                    <video
                                ...
                                <div className="absolute left-2 top-2 inline-flex items-center gap-1.5 rounded bg-black/65 px-1.5 py-[3px] font-mono text-chrome-sm font-medium uppercase text-foreground">
                                    ...
                                    <span className="text-foreground/90">{task.model || "?"}</span>
                                    {task.is_starred ? (
                ...
                {/* Footer controls */}
                <footer className="flex shrink-0 items-center gap-3 border-t border-foreground/10 px-4 py-3">
                    <button
                        type="button"
                        onClick={togglePlay}
                        aria-label={isPlaying ? "Pause" : "Play"}
                        className="grid h-10 w-10 place-items-center rounded-full bg-foreground/10 text-foreground transition-colors duration-fast ease-out-quart hover:bg-foreground/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/55"
                    >
