...
import { api, type EnvConfigPayload, type MediaProviderMode, type ProviderMode } from "@/lib/api";
...
type EnvConfig = EnvConfigPayload & {
  ...
  GPT_IMAGE_PROVIDER_MODE: MediaProviderMode;
  SEEDANCE_PROVIDER_MODE: MediaProviderMode;
...
const DEFAULT_CONFIG: EnvConfig = {
  ...
  NEWAPI_BASE_URL: "https://aiapi.zqgame.com/v1",
  NEWAPI_CHAT_API_KEY: "",
  NEWAPI_CHAT_MODEL: "qwen3.7-max",
  NEWAPI_IMAGE_API_KEY: "",
  NEWAPI_IMAGE_MODEL: "gpt-image-2",
  NEWAPI_VIDEO_API_KEY: "",
  NEWAPI_VIDEO_MODEL: "doubao-seedance-2-0-260128",
  GPT_IMAGE_PROVIDER_MODE: "mulerouter",
...
const normalizeMediaProviderMode = (mode?: string): MediaProviderMode =>
  mode === "newapi" ? "newapi" : "mulerouter";
...
const normalizeEnvConfig = (existing: EnvConfig, data?: EnvConfigPayload): EnvConfig => ({
  ...
  GPT_IMAGE_PROVIDER_MODE: normalizeMediaProviderMode(
    data?.GPT_IMAGE_PROVIDER_MODE ?? existing.GPT_IMAGE_PROVIDER_MODE,
  ),
  SEEDANCE_PROVIDER_MODE: normalizeMediaProviderMode(
    data?.SEEDANCE_PROVIDER_MODE ?? existing.SEEDANCE_PROVIDER_MODE,
  ),
...
const getValidationErrors = (env: EnvConfig): string[] => {
  ...
  const hasNewApiChat = Boolean(
    env.LLM_PROVIDER === "newapi"
      && env.NEWAPI_BASE_URL?.trim()
      && env.NEWAPI_CHAT_API_KEY?.trim()
      && env.NEWAPI_CHAT_MODEL?.trim(),
  );
  if (!hasDashScope && !hasNewApiChat) {
    errors.push("DashScope key or complete New API chat configuration");
  }
              ...
              <>
                <div>
                  <label className="flex items-center justify-between text-sm font-medium text-foreground mb-2">
                    <span>DashScope API Key <span className="text-text-muted text-xs">(optional with New API)</span></span>
                  ...
                  <input
                    ...
                    placeholder="Leave empty when using New API for chat"
                ...
                <div className="pt-4 border-t border-glass-border">
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <h3 className="text-sm font-bold text-foreground">OpenAI-compatible / New API</h3>
                      <p className="text-[0.6875rem] text-text-muted mt-1">
                        Uses separate chat, image, and video tokens. Keys are never sent to DashScope or MuleRouter.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-glass border border-glass-border rounded-lg p-4 space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-2">Chat provider</label>
                      <div className="flex flex-wrap gap-2">
                        <button
                          type="button"
                          onClick={() => handleChange("LLM_PROVIDER", "dashscope")}
                          className={modeButtonClass(config.LLM_PROVIDER !== "newapi")}
                        >
                          DashScope
                        </button>
                        <button
                          type="button"
                          onClick={() => handleChange("LLM_PROVIDER", "newapi")}
                          className={modeButtonClass(config.LLM_PROVIDER === "newapi")}
                        >
                          New API
                        </button>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-2">New API Base URL</label>
                      <input
                        type="url"
                        value={config.NEWAPI_BASE_URL}
                        onChange={(e) => handleChange("NEWAPI_BASE_URL", e.target.value)}
                        placeholder="https://aiapi.zqgame.com/v1"
                        className={inputClass}
                      />
                    </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">Chat model</label>
                        <input
                          type="text"
                          value={config.NEWAPI_CHAT_MODEL}
                          onChange={(e) => handleChange("NEWAPI_CHAT_MODEL", e.target.value)}
                          placeholder="qwen3.7-max"
                          className={inputClass}
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">Chat API Key</label>
                        <input
                          type="password"
                          value={config.NEWAPI_CHAT_API_KEY}
                          onChange={(e) => handleChange("NEWAPI_CHAT_API_KEY", e.target.value)}
                          placeholder="sk-..."
                          className={inputClass}
                        />
                      </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">Image model</label>
                        <input
                          type="text"
                          value={config.NEWAPI_IMAGE_MODEL}
                          onChange={(e) => handleChange("NEWAPI_IMAGE_MODEL", e.target.value)}
                          placeholder="gpt-image-2"
                          className={inputClass}
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">Image API Key</label>
                        <input
                          type="password"
                          value={config.NEWAPI_IMAGE_API_KEY}
                          onChange={(e) => handleChange("NEWAPI_IMAGE_API_KEY", e.target.value)}
                          placeholder="sk-..."
                          className={inputClass}
                        />
                      </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">Video model</label>
                        <input
                          type="text"
                          value={config.NEWAPI_VIDEO_MODEL}
                          onChange={(e) => handleChange("NEWAPI_VIDEO_MODEL", e.target.value)}
                          placeholder="doubao-seedance-2-0-260128"
                          className={inputClass}
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">Video API Key</label>
                        <input
                          type="password"
                          value={config.NEWAPI_VIDEO_API_KEY}
                          onChange={(e) => handleChange("NEWAPI_VIDEO_API_KEY", e.target.value)}
                          placeholder="sk-..."
                          className={inputClass}
                        />
                      </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">GPT Image route</label>
                        <div className="flex gap-2">
                          <button
                            type="button"
                            onClick={() => handleChange("GPT_IMAGE_PROVIDER_MODE", "mulerouter")}
                            className={modeButtonClass(config.GPT_IMAGE_PROVIDER_MODE === "mulerouter")}
                          >
                            MuleRouter
                          </button>
                          <button
                            type="button"
                            onClick={() => handleChange("GPT_IMAGE_PROVIDER_MODE", "newapi")}
                            className={modeButtonClass(config.GPT_IMAGE_PROVIDER_MODE === "newapi")}
                          >
                            New API
                          </button>
                        </div>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-2">Seedance route</label>
                        <div className="flex gap-2">
                          <button
                            type="button"
                          ...
                          <button
                            type="button"
                            onClick={() => handleChange("SEEDANCE_PROVIDER_MODE", "newapi")}
                            className={modeButtonClass(config.SEEDANCE_PROVIDER_MODE === "newapi")}
                          >
                            New API
                          </button>
                        </div>
